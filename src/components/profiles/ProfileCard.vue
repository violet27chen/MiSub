...
const emit = defineEmits(['delete', 'change', 'edit', 'open-copy', 'preview', 'move-up', 'move-down', 'view-logs', 'qrcode', 'duplicate']);
  ...
  >
    <div class="flex items-start justify-between gap-3" :class="compact ? 'flex-col' : ''">
      ...
      <div class="shrink-0 flex items-center gap-1 opacity-100 transition-opacity duration-200" :class="compact ? 'self-end flex-wrap justify-end' : ''">
		...
		<button @click.stop="emit('duplicate')" class="p-2.5 rounded-full hover:bg-primary-50 dark:hover:bg-white/10 text-gray-400 hover:text-primary-500 transition-colors min-w-[44px] min-h-[44px] lg:min-w-0 lg:min-h-0 flex items-center justify-center" :title="t('actions.duplicateProfile')">
