...
const prependGroupName = profilePrefixSettings?.prependGroupName ?? false;
    ...
    const fetchSingleSubscription = async (sub) => {
        ...
        try {
            ...
            // 在作为订阅被重新分组前按段过滤掉，避免输出变成 "机场名 - 手动节点 - 实际名"。
            const manualNodePrefix = profilePrefixSettings?.manualNodePrefix || '手动节点';
            const stripManualSegments = (url) => {
                const hashIdx = url.indexOf('#');
                if (hashIdx === -1) return url;
                const before = url.slice(0, hashIdx + 1);
                const after = url.slice(hashIdx + 1);
                const parts = after.split(' - ');
                const filtered = parts.filter(p => p !== manualNodePrefix);
                return before + filtered.join(' - ');
            };
            validNodes = validNodes.map(stripManualSegments);
